...
data.ncl <- ubNCL(data.tra, labels.tra)

model.NCL.RF <- model.rf(data.ncl$X, data.ncl$Y, params = c(ntree = 500))
preds.NCL.RF <- pred.rf(model = model.NCL.RF, test = data.tst)
writePredictions(preds.NCL.RF, "NCL.RF")


model.NCL.SVM <- model.rf(data.ncl$X, data.ncl$Y, params = c(kernel = "radial"))
preds.NCL.SVM <- pred.rf(model = model.NCL.SVM, test = data.tst)
writePredictions(preds.NCL.SVM, "NCL.SVM")

poll <- apply(matrix(c(preds.NCL.RF, preds.NCL.SVM),ncol=2), 1, mean)
writePredictions(poll, "NCL.Poll")
