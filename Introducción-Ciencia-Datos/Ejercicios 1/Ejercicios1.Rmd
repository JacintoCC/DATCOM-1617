...
### Busca que hace la función `sequence()`. ¿Cuál es la diferencia con la función `seq()`?
   ...
   La función `sequence(n)` genera el vector `1:n`. Sería equivalente a la función `seq(1, n)`. La función `seq` permite generar una secuencia a partir de múltiples parámetros.
...
```{r}
scan("iris.csv", what = character(), sep =";")
```
