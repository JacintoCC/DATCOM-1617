...
data.tra <- data.tra %>%
   ...
   mutate(MES = month.names.2.numeric[MES],
          ...
          ACERAS = as.factor(ifelse(is.na(ACERAS),
                                    ifelse(PRIORIDAD == "PASO PARA PEATONES", "SI HAY ACERA", NA),
                                           levels(ACOND_CALZADA)[ACOND_CALZADA])),
          NUMBER.NA = apply(., 1,  function(x) sum(is.na(x)))) %>%
   mice::mice(m=3, method="pmm", maxit = 1) %>%
   mice::complete()
...
rows.3.NA <- apply(filter(data.na$df, NUMBER.NA==3), 2, function(x) mean(x=="TRUE")) 
Filter(function(x) x > 0,
       rows.3.NA)
...
dist.diff.1 <- filter(data.frame(Name.var = colnames(data.tra),
                                 Name.value = sapply(dist.diff.1, function(x) names(x)[which.max(x)]),
                                 diff = sapply(dist.diff.1, max)), diff > 0)
dist.diff.1[which.max(dist.diff.1$diff[-c(29,30)]), ]
...
data.tst.imp <- data.tst %>% 
   ...
   mutate(MES = month.names.2.numeric[MES],
          ...
          ACERAS = as.factor(ifelse(is.na(ACERAS),
                                    ...
                                    levels(ACOND_CALZADA)[ACOND_CALZADA])),
          NUMBER.NA = apply(., 1,  function(x) sum(is.na(x)))) %>%
   rbind(select(data.tra, -TIPO_ACCIDENTE)) %>%
   cbind("Is.Test" = c(rep(T, nrow(data.tst)),
                       rep(F, nrow(data.tra)))) %>%
   mice::mice(m=4, method="pmm", maxit = 1) %>%
   mice::complete()
data.tst.imp <- data.tst.imp %>%
   filter(Is.Test) %>%
   select(-Is.Test) 
...
set.seed(3141592)
model <- randomForest(select(data.tra, -TIPO_ACCIDENTE),
                      data.tra$TIPO_ACCIDENTE,
                      cutoff = sapply(levels(data.tra$TIPO_ACCIDENTE), 
                                      function(x) mean(x==data.tra$TIPO_ACCIDENTE)))

#' Predicción sobre datos de test
#' Escritura de los datos
predictions <- predict(model, data.tst.imp)
writePredictions(predictions, "31-3-RF")
