...
#' eta = c(0.1, 0.3)
set.seed(3141592)
sapply(levels(data.tra$TIPO_ACCIDENTE), function(l) mean(l == data.tra$TIPO_ACCIDENTE)) -> x
results <- cross.validation(data   = select(data.tra, -TIPO_ACCIDENTE),
                            ...
                            params = expand.grid(cutoff = unname(x)))
...
writePredictions(predictions, "31-2-Mice")
